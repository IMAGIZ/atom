...
class QubitInformationObjectXmlCache
{
   ...
   * @param string  format of XML ('dc', 'ead', or 'all')
   *
   * @return null
   */
  public function export($resource, $format = 'all')
  {
  ...
  public function exportAll($options = array())
  {
    ...
    // Use format if specified and valid; otherwise, permissively default to 'all'
    $valid_formats = array('dc', 'ead');
    $format = isset($options['format']) && in_array($options['format'], $valid_formats) ? $options['format'] : 'all';
